...
DROP POLICY IF EXISTS "locations_mutation_policy" ON public.locations;
CREATE POLICY "locations_mutation_policy" ON public.locations
  FOR ALL
  USING (TRUE)
  WITH CHECK (TRUE);
...
-- INSERT: Allow licensed surveyors or public app submissions in prototype mode
DROP POLICY IF EXISTS "buildings_insert_surveyor" ON public.buildings;
DROP POLICY IF EXISTS "buildings_insert_public" ON public.buildings;
CREATE POLICY "buildings_insert_public" ON public.buildings
  FOR INSERT
  WITH CHECK (TRUE);

-- UPDATE:
DROP POLICY IF EXISTS "buildings_update_governance" ON public.buildings;
DROP POLICY IF EXISTS "buildings_update_public" ON public.buildings;
CREATE POLICY "buildings_update_public" ON public.buildings
  FOR UPDATE
  USING (TRUE)
  WITH CHECK (TRUE);
