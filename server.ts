...
import { startKeepAlivePing } from './src/utils/keepAlive';
...
async function startServer() {
  ...
  // Google Maps URL Resolver (Expands shortlinks maps.app.goo.gl and extracts lat/lng)
  app.post('/api/resolve-maps-url', async (req, res) => {
    try {
      ...
      function extractCoords(str: string): { lat: number; lng: number; source: string } | null {
        try {
        ...
        // Pattern 3: ?q=lat,lng or query=lat,lng or ll=lat,lng or loc:lat,lng
        const qMatch = str.match(/(?:[?&](?:q|query|ll|sll|daddr)=|loc:)(-?\d+\.?\d*)[,\s+]+(-?\d+\.?\d*)/i);
        if (qMatch) {
        ...
        const pathMatch = str.match(/\/(?:place|dir)\/[^/]*?(-?\d+\.\d+)[,\s+]+(-?\d+\.\d+)/);
        if (pathMatch) {
          const lat = parseFloat(pathMatch[1]), lng = parseFloat(pathMatch[2]);
          if (!isNaN(lat) && !isNaN(lng) && lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180) {
            return { lat, lng, source: 'Google Maps Path Coordinates' };
          }
        }

        // Pattern 5: Direct numerical string "17.4485, 78.3748"
        const directMatch = str.match(/^(-?\d+\.?\d*)[,\s;]+(-?\d+\.?\d*)$/);
        if (directMatch) {
          const lat = parseFloat(directMatch[1]), lng = parseFloat(directMatch[2]);
          if (!isNaN(lat) && !isNaN(lng) && lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180) {
      ...
      let extracted = extractCoords(url);
      if (extracted) {
        return res.json({ success: true, ...extracted, resolvedUrl: url });
      }

      // 2. Shortlink redirect resolution
      let targetUrl = url;
      ...
      try {
        ...
        extracted = extractCoords(finalUrl) || extractCoords(html);
      } catch (fetchErr: any) {
        console.warn('Google Maps redirect fetch error:', fetchErr.message);
      }

      if (extracted) {
        return res.json({ success: true, ...extracted, resolvedUrl: finalUrl });
      }
