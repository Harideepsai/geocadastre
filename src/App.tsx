...
export default function App() {
  ...
  // Front-door security gate state
  const [isDoorUnlocked, setIsDoorUnlocked] = useState<boolean>(() => {
    if (typeof window === 'undefined') return false;
    return (
      sessionStorage.getItem('isDoorUnlocked') === 'true' ||
      localStorage.getItem('isDoorUnlocked') === 'true'
    );
  });
  ...
  // Front-door guard: block unauthorized access until door lock credentials are valid
  if (!isDoorUnlocked) {
    return <SecurityGateOverlay onUnlock={() => setIsDoorUnlocked(true)} />;
  }
