...
export class CadastreService {
  ...
  async ingestBuilding(payload: IngestionPayload): Promise<{
    ...
    for (let f = 0; f < totalFloors; f++) {
      ...
      const flrId = `FLR-${bldCode}-F${f.toString().padStart(2, '0')}-${runTag}`;
      newFloors.push({
        id: flrId,
        floor_id: flrId,
        building_id: bldCode,
    ...
    for (let f = 0; f < totalFloors; f++) {
      ...
      for (let u = 1; u <= unitsPerFloor; u++) {
        ...
        const propId = `PROP-${bldCode}-F${f}-U${u}-${runTag}`;
        const unitId = `UNIT-${bldCode}-${f}-${u}-${runTag}`;
        ...
        newGeometries.push({
          id: `GEOM-${bldCode}-${f}-${u}-${runTag}`,
          geometry_id: `GEOM-${bldCode}-${f}-${u}-${runTag}`,
          property_id: unitId,
        ...
        // Owner & Title Record
        const ownerId = `OWN-${bldCode}-${f}-${u}-${runTag}`;
        const ownerNames = [
        ...
        newOwnerships.push({
          id: `OWNP-${bldCode}-${f}-${u}-${runTag}`,
          ownership_id: `OWNP-${bldCode}-${f}-${u}-${runTag}`,
          property_id: unitId,
          owner_id: ownerId,
          ownership_share: 100,
        ...
        newRecords.push({
          id: `REC-${bldCode}-${f}-${u}-${runTag}`,
          record_id: `REC-${bldCode}-${f}-${u}-${runTag}`,
          property_id: unitId,
          source_reference: 'IGRS / Authorized Property Record',
    ...
    const uniqueLocId = `LOC-${bldId}-${Date.now().toString(36)}`;
    const newLocation: Location = {
      id: uniqueLocId,
      location_id: uniqueLocId,
      building_id: bldCode,
      latitude: payload.coordinates.lat,
      longitude: payload.coordinates.lng,
    ...
    if (client && isSupabaseConfigured) {
      try {
        const { error: bldErr } = await client.from('buildings').insert([
          {
            id: newBuilding.id,
            ...
            status: newBuilding.status || 'draft',
          },
        ]);

        if (bldErr) {
          console.error('❌ Supabase buildings insert error:', bldErr.message, bldErr.details);
        } else {
          console.log('✅ Successfully inserted building into Supabase:', newBuilding.id);
        }

        if (newLocation) {
          const { error: locErr } = await client.from('locations').insert([
            {
              id: newLocation.id,
              building_id: newBuilding.id,
              latitude: newLocation.latitude,
              longitude: newLocation.longitude,
              formatted_address: newLocation.address || newBuilding.address,
              state: newLocation.state || 'Telangana',
              district: newLocation.city || 'Hyderabad',
              pincode: newLocation.pincode || '500081',
            },
          ]);
          if (locErr) console.warn('❌ Supabase locations insert error:', locErr.message, locErr.details);
          else console.log('✅ Successfully inserted location into Supabase');
        }

        if (newFloors && newFloors.length > 0) {
          const floorsMapped = newFloors.map((flr) => ({
            id: flr.id,
            building_id: newBuilding.id,
            floor_number: flr.floor_number,
            bottom_height: flr.bottom_height,
            top_height: flr.top_height,
            floor_name: flr.floor_name || `Floor ${flr.floor_number}`,
          }));
          const { error: flrErr } = await client.from('floors').insert(floorsMapped);
          if (flrErr) console.warn('❌ Supabase floors insert error:', flrErr.message, flrErr.details);
          else console.log(`✅ Successfully inserted ${floorsMapped.length} floors into Supabase`);
        }

        if (newUnits && newUnits.length > 0) {
          const unitsMapped = newUnits.map((u) => ({
            id: u.id,
            building_id: newBuilding.id,
            floor_id: u.floor_id,
          ...
          else console.log(`✅ Successfully inserted ${unitsMapped.length} property units into Supabase`);
        }

        if (newGeometries && newGeometries.length > 0) {
          const geometriesMapped = newGeometries.map((g) => ({
            id: g.id,
            property_id: g.property_id,
            bottom_height: g.bottom_height,
            top_height: g.top_height,
            width: g.width,
            length: g.length,
            height: g.height,
            x_offset: g.x_offset || 0,
            y_offset: g.y_offset || 0,
          }));
          const { error: geomErr } = await client.from('vertical_geometries').insert(geometriesMapped);
          if (geomErr) console.warn('❌ Supabase vertical_geometries insert error:', geomErr.message, geomErr.details);
          else console.log(`✅ Successfully inserted ${geometriesMapped.length} vertical geometries into Supabase`);
        }

        if (newPrototypeIds && newPrototypeIds.length > 0) {
          const prototypeIdsMapped = newPrototypeIds.map((p) => ({
            id: p.id,
            property_id: p.property_id,
            generated_id: (p as any).generated_identifier || (p as any).generated_id || (p as any).standard_3d_ulpin || p.id,
            format_pattern: p.format_pattern || '{STATE}-{BUILDING}-{FLOOR}-{UNIT}',
            status: p.status || 'PROTOTYPE_ACTIVE',
          }));
          const { error: protoErr } = await client.from('prototype_3d_property_ids').insert(prototypeIdsMapped);
          if (protoErr) console.warn('❌ Supabase prototype_3d_property_ids insert error:', protoErr.message, protoErr.details);
          else console.log(`✅ Successfully inserted ${prototypeIdsMapped.length} prototype 3D property IDs into Supabase`);
        }

        if (newOwners && newOwners.length > 0) {
          const ownersMapped = newOwners.map((o) => ({
            id: o.id,
            owner_name: o.owner_name,
            contact_info: o.contact_info,
            id_proof_type: o.id_proof_type || 'Aadhaar / Digital Land Registry Card',
          }));
          const { error: ownErr } = await client.from('owners').insert(ownersMapped);
          if (ownErr) console.warn('❌ Supabase owners insert error:', ownErr.message, ownErr.details);
          else console.log(`✅ Successfully inserted ${ownersMapped.length} owners into Supabase`);
        }

        if (newOwnerships && newOwnerships.length > 0) {
          const ownershipsMapped = newOwnerships.map((o) => ({
            id: o.id,
            property_id: o.property_id,
            owner_id: o.owner_id,
            ownership_share: o.ownership_share || 100,
            ownership_type: o.ownership_type || 'Sole Owner',
          }));
          const { error: ownshipErr } = await client.from('ownerships').insert(ownershipsMapped);
          if (ownshipErr) console.warn('❌ Supabase ownerships insert error:', ownshipErr.message, ownshipErr.details);
          else console.log(`✅ Successfully inserted ${ownershipsMapped.length} ownerships into Supabase`);
        }

        if (newRecords && newRecords.length > 0) {
          const recordsMapped = newRecords.map((r) => ({
            id: r.id,
            property_id: r.property_id,
            source_type: 'IGRS / Authorized Property Record',
            survey_number: r.survey_number,
            document_reference: r.document_reference,
            area: r.area,
            address: r.address,
            registration_date: r.registration_date || new Date().toISOString().split('T')[0],
            sub_registrar_office: r.sub_registrar_office || 'Sub-Registrar Office, Madhapur, Rangareddy',
            market_value: r.market_value || 5000000,
          }));
          const { error: recErr } = await client.from('property_records').insert(recordsMapped);
          if (recErr) console.warn('❌ Supabase property_records insert error:', recErr.message, recErr.details);
          else console.log(`✅ Successfully inserted ${recordsMapped.length} property records into Supabase`);
        }
      } catch (dbErr: any) {
        console.warn('Error inserting into Supabase tables, saving locally:', dbErr?.message || dbErr);
      }
    }
  ...
  async registerCadastre(
    ...
    if (client && isSupabaseConfigured) {
      try {
        // 1. Update Buildings status and deed reference
        const { error: bldErr } = await client
          .from('buildings')
          .update({
            status: 'registered',
            deed_reference: deedReference,
          ...
          .or(`id.eq.${bld.id},building_id.eq.${targetBldId}`);
        if (bldErr) console.warn('Supabase building status update note:', bldErr.message);

        // 2. Upsert Location for this building
        const loc = cache.locations.find((l) => l.building_id === targetBldId || l.building_id === bld.id);
        if (loc) {
          await client.from('locations').upsert([
            {
              ...
              formatted_address: loc.address || bld.address,
              state: loc.state || 'Telangana',
              district: loc.city || 'Hyderabad',
              pincode: loc.pincode || '500081',
            },
          ]);
        }

        // 3. Upsert Floors for this building
        const bldFloors = cache.floors.filter((f) => f.building_id === targetBldId || f.building_id === bld.id);
        if (bldFloors.length > 0) {
          await client.from('floors').upsert(
            bldFloors.map((flr) => ({
              id: flr.id,
              building_id: bld.id,
              floor_number: flr.floor_number,
        ...
        // 4. Upsert Property Units
        if (units.length > 0) {
          await client.from('property_units').upsert(
            units.map((u) => ({
              id: u.id,
              building_id: bld.id,
              floor_id: u.floor_id,
              ...
              area: u.area,
              property_type: u.property_type || 'Residential Apartment',
              record_reference: deedReference,
            }))
          );

          const unitIds = units.map((u) => u.property_id || u.id);

          // 5. Upsert Vertical Geometries
          const bldGeoms = cache.geometries.filter((g) => unitIds.includes(g.property_id));
          if (bldGeoms.length > 0) {
            await client.from('vertical_geometries').upsert(
              bldGeoms.map((g) => ({
                id: g.id,
                property_id: g.property_id,
                bottom_height: g.bottom_height,
          ...
          // 6. Upsert Prototype 3D Property ULPINs
          const bldP3d = cache.prototypeIds.filter((p) => unitIds.includes(p.property_id));
          if (bldP3d.length > 0) {
            await client.from('prototype_3d_property_ids').upsert(
              bldP3d.map((p) => ({
                id: p.id,
                property_id: p.property_id,
                generated_id: (p as any).generated_identifier || (p as any).generated_id || p.id,
                format_pattern: p.format_pattern || '{STATE}-{BUILDING}-{FLOOR}-{UNIT}',
                status: 'PROTOTYPE_ACTIVE',
              }))
            );
          ...
          // 7. Upsert Ownerships & Owners
          const bldOwnerships = cache.ownerships.filter((o) => unitIds.includes(o.property_id));
          if (bldOwnerships.length > 0) {
            const ownerIds = bldOwnerships.map((o) => o.owner_id);
            const bldOwners = cache.owners.filter((ow) => ownerIds.includes(ow.id) || ownerIds.includes(ow.owner_id));
            if (bldOwners.length > 0) {
              await client.from('owners').upsert(
                bldOwners.map((ow) => ({
                  id: ow.id,
                  owner_name: ow.owner_name,
                  contact_info: ow.contact_info,
                  id_proof_type: ow.id_proof_type || 'Aadhaar / Digital Land Registry Card',
                }))
              );
            }

            await client.from('ownerships').upsert(
              bldOwnerships.map((o) => ({
                id: o.id,
                property_id: o.property_id,
                owner_id: o.owner_id,
          ...
          // 8. Upsert Property Records
          const bldRecords = cache.records.filter((r) => unitIds.includes(r.property_id));
          if (bldRecords.length > 0) {
            await client.from('property_records').upsert(
              bldRecords.map((r) => ({
                id: r.id,
                property_id: r.property_id,
                source_type: 'IGRS / Authorized Property Record',
                survey_number: r.survey_number || bld.survey_number,
                document_reference: deedReference,
                area: r.area,
                address: r.address,
                registration_date: r.registration_date || new Date().toISOString().split('T')[0],
                sub_registrar_office: r.sub_registrar_office || 'Sub-Registrar Office, Madhapur, Rangareddy',
        ...
        console.log(`✅ Successfully synced 3D Cadastre SRO Lock & ULPIN to Supabase for ${targetBldId}`);
      } catch (e: any) {
        console.warn('Supabase update note on SRO Lock:', e?.message || e);
      }
    }
    ...
    if (client && isSupabaseConfigured) {
      try {
        await client.from('property_units').upsert({
          ...
          area: unit.area,
          record_reference: unit.property_record_ref || 'IGRS Deed',
        });
        ...
        if (geom) {
          await client.from('vertical_geometries').upsert({
            ...
            length: geom.length,
            height: geom.height,
            x_offset: geom.x_offset || 0,
            y_offset: geom.y_offset || 0,
          });
        }
        ...
        if (p3d) {
          await client.from('prototype_3d_property_ids').upsert({
            id: p3d.id,
            property_id: unit.property_id || unit.id,
            generated_id: (p3d as any).generated_identifier || (p3d as any).generated_id || p3d.id,
            format_pattern: p3d.format_pattern || '{STATE}-{BUILDING}-{FLOOR}-{UNIT}',
            status: 'PROTOTYPE_ACTIVE',
          });
        }
        ...
        if (owner) {
          await client.from('owners').upsert({
            ...
            id_proof_type: owner.id_proof_type || 'Aadhaar / Digital Land Registry Card',
          });
        }
        ...
        if (ownership) {
          await client.from('ownerships').upsert({
            ...
            ownership_share: ownership.ownership_share || 100,
            ownership_type: ownership.ownership_type || 'Sole Owner',
          });
        }
        console.log(`✅ Successfully updated flat/owner/ULPIN in Supabase for property ${unit.property_id}`);
      } catch (e: any) {
        console.warn('Supabase sync note on flat details update:', e?.message || e);
      }
    }
